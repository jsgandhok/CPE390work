...
Body::Body(const char name[16], const char orbits[16], double m, double x,
    ...
    : m(m), Gm(G * m), pos(x, y, z), v(vx, vy, vz), a() {
...
GravSim::GravSim(const char filename[]) {
  ...
  while (!f.eof()) {
    ...
    double x = r, y = 0, z = 0;  // replcing the trig 0 vals with actual output
                                 // cos 0 = 1  &  sin 0 = 0
                                 // make x = r ... x = r * 1 not needed
    // handle special case for sun: if r = 0, then v = 0
    // make y = 0 ... y = r * 0 not needed
    double v = r == 0 ? 0 : sqrt(G * m * (2 / r - 1 / aphelion));

    double vx = 0, vy = v,
           vz = 0;  // same code optimizing logic used from lines 55-59
    bodies.push_back(
...
inline Vec3d Body::gravAccel(const Body& b)
  ...
  double r = 1 / dpos.magsq();  // do not sqrt 1 / sumofxyz square
  double tm = sqrt(r);
  // double inv_r = 1 / r;  // inverse r for faster mult. computations in
  // re optimized and don;t need the above line
  // return Vec3d statement
  double amag = b.Gm * r * tm;  // precomputed Gm and only a single division
  return Vec3d(dpos.x * amag, dpos.y * amag,
               dpos.z * amag);  // mult inverse instead of division
}
...
void GravSim::timestep(double dt) {
  for (int i = 0; i < bodies.size(); i++) {
    ...
    for (int j = 0; j < bodies.size(); j++)
      if (i != j)
        a += bodies[i].gravAccel(bodies[j]);  // This calls operator + in class
                                              // Vec3d. inlining might be faster
                                              // += or = a + which is faster

    // NOTE: += IS MUCH FASTER THAN a+
    // went from 5 sec to 2 sec!!!
    bodies[i].a = a;
  }

  for (int i = 0; i < bodies.size(); i++) {
    bodies[i].pos += ((bodies[i].a * dt + bodies[i].v) * dt + 0.5);
    // above is  "+_ implementation" of this code below:
    // bodies[i].pos = ((bodies[i].a * dt + bodies[i].v) * dt + bodies[i].pos +
    // 0.5);
    // Hint: Horner's form is faster
